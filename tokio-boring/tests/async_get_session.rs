use boring_vendit::ssl::{SslOptions, SslRef, SslSession, SslSessionCacheMode, SslVersion};
use futures::future;
use std::sync::atomic::{AtomicBool, Ordering};
use std::sync::OnceLock;
use tokio::net::TcpStream;
use tokio::task::yield_now;
use tokio_boring_vendit::BoxGetSessionFinish;
...
async fn test() {
    ...
    let server = async move {
        tokio_boring_vendit::accept(&acceptor, listener.accept().await.unwrap().0)
            .await
            .unwrap();
