...
use boring_vendit::ssl::{SslAcceptor, SslConnector, SslFiletype, SslMethod};
use bytes::Bytes;
use futures::StreamExt;
use http_body_util::{BodyStream, Empty};
use hyper::{service, Response};
use hyper_boring_vendit::v1::HttpsConnector;
...
async fn localhost() {
    ...
    let server = async move {
        ...
        for _ in 0..3 {
            ...
            let stream = tokio_boring_vendit::accept(&acceptor, stream)
                .await
                .unwrap();
...
#[tokio::test]
async fn alpn_h2() {
    use boring_vendit::ssl::{self, AlpnError};
